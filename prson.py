...
def execute_commands(input_variable: str, commands_file: str, type: str):
    ...
    # Executes commands, replacing $INPUT with argv[1] and $OUTPUT_DIR with output_directory variable
...
def fill_out_form(link: str, field_attribute: str, field_value: str, field_input: str, driver: webdriver.Firefox):
    driver.get(url=link)
...
def open_new_tab(driver: webdriver.Firefox):
...
def read_websites_file_and_visit(websites_file: str, type: str, input_str: str) -> list:
    try:
...
def lookup_email(email: str, only_commands: bool, only_websites: bool):
    if ask_questions:
            ...
            if ask_polar_question('Do you want to execute system commands in order to perform an OSINT lookup of the provided email address?'):
                ...
                if not only_commands:
                    if ask_polar_question('Do you want to start Firefox in order to perform online information gathering?'):
                        read_websites_file_and_visit(websites_file, 'email', email)
        else:
            if not only_commands:
                if ask_polar_question('Do you want to start Firefox in order to perform online information gathering?'):
                    read_websites_file_and_visit(websites_file, 'email', email)
...
def lookup_phone_number(number: str, only_commands: bool, only_websites: bool):
    if ask_questions:
        if not only_websites:
            if ask_polar_question('Do you want to execute system commands in order to perform an OSINT lookup of the provided telephone number?'):
                ...
                if not only_commands:
                    if ask_polar_question('Do you want to start Firefox in order to perform online information gathering?'):
                        read_websites_file_and_visit(websites_file, 'phone', number)
        else:
            if not only_commands:
                if ask_polar_question('Do you want to start Firefox in order to perform online information gathering?'):
                    read_websites_file_and_visit(websites_file, 'phone', number)
...
# Looks up a username
def lookup_username(username: str, only_commands: bool, only_websites: bool):
    if ask_questions:
        if not only_websites:
            if ask_polar_question('Do you want to execute system commands in order to perform an OSINT lookup of the provided username?'):
                ...
                if not only_commands:
                    if ask_polar_question('Do you want to start Firefox in order to perform online information gathering?'):
                        read_websites_file_and_visit(websites_file, 'username', username)
        else:
            if not only_commands:
                if ask_polar_question('Do you want to start Firefox in order to perform online information gathering?'):
                        read_websites_file_and_visit(websites_file, 'username', username)
...
def main():
    ...
    try:
        ...
        if args.email or args.number or args.username:
            if args.email:
                lookup_email(args.input, args.commands, args.websites)
            elif args.number:
                lookup_phone_number(args.input, args.commands, args.websites)
            else:
                lookup_username(args.input, args.commands, args.websites)
        # Evaluate automatically type of input string and act accordingly
        else: 
            if is_email(args.input):
                lookup_email(args.input, args.commands, args.websites)
            elif is_phone_number(args.input) or args.input.replace('+', '').isnumeric():
                lookup_phone_number(args.input, args.commands, args.websites)
            else:
                lookup_username(args.input, args.commands, args.websites)
